...
export default function PlayerTypeSelection() {
  ...
  const currentQ = {
    ...
    emoji: "🎭",
    options: playerTypeOptions.map(option => ({
      text: option.label,
      emoji: "👤",
      icon: null
    }))
  ...
  return (
    <div className="w-full mx-auto h-screen bg-gradient-to-br from-gray-900 via-purple-900 to-violet-900 text-white relative overflow-y-auto">
      ...
      <div className="absolute  right-10 w-32 h-32 bg-purple-500/10 rounded-full blur-3xl animate-pulse" />
      ...
      {/* Question Content */}
      <div className="px-4 sm:px-6 md:px-8 lg:px-10 xl:px-12 pt-10 pb-4 sm:pb-6 md:pb-8 lg:pb-10">

        <div className="space-y-1 sm:space-y-2 md:space-y-3">
          {currentQ.options.map((option, index) => {
            const isSelected = gameHabit === playerTypeOptions[index]?.id;
            return (
              <button
                key={index}
                onClick={() => handleSelectGameHabit(playerTypeOptions[index].id)}
              ...
              >
                <div className="flex items-center gap-2 sm:gap-3">
                  ...
                  <div className="flex-1">
                    <span className="text-sm sm:text-base md:text-lg lg:text-xl font-medium leading-[1]">{option.text}</span>
                  </div>
