...
export default function GenderSelection() {
  ...
  const { gender, setGender, setCurrentStep, currentStep } = useOnboardingStore()
      ...
      <div className="px-4 sm:px-6 md:px-8 lg:px-10 xl:px-12 pt-10 pb-8 sm:pb-10 md:pb-12 lg:pb-14">
        ...
        <div className="space-y-2">
          {currentQ.options.map((option, index) => {
            const isSelected = gender === genderOptions[index]?.id;
            return (
              <button
                key={index}
                onClick={() => handleGenderSelect(genderOptions[index].id)}
