...
// Confirm Apple IAP payment
export const confirmApplePayment = createAsyncThunk(
  'vip/confirmApplePayment',
  async (
    { subscriptionId, transactionId, transactionReceipt, jwsRepresentation, productId, token },
    { rejectWithValue },
  ) => {
    try {
      const response = await verifyApplePurchase(
        {
          subscriptionId,
          transactionId,
          transactionReceipt,
      ...
      if (response.success === false) {
        throw new Error(
          response.message ||
            response.error ||
            'Apple payment verification failed',
        )
      }

      return response
    } catch (error) {
      let errorMessage = 'Apple payment verification failed'
...
const vipSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(confirmApplePayment.pending, (state) => {
        state.purchaseStatus = "loading";
        state.purchaseError = null;
      })
      .addCase(confirmApplePayment.fulfilled, (state, action) => {
        state.purchaseStatus = "succeeded";
        state.purchaseError = null;
        state.activeSubscriptionId = null;
        state.appleProductId = null;
      })
      .addCase(confirmApplePayment.rejected, (state, action) => {
        state.purchaseStatus = "failed";
        state.purchaseError = action.payload;
      })
      .addCase("vip/initiateApplePurchase/fulfilled", (state, action) => {
        state.purchaseStatus = "awaiting_payment";
        state.appleProductId = action.payload.appleProductId;
        state.activeSubscriptionId = action.payload.subscriptionId;
      });
  },
});
