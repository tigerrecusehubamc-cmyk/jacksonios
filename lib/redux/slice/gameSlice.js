...
const initialState = {
  ...
  gamesBySectionOwner: {}, // User ID that owns each section cache
  gamesBySectionRequestOwner: {}, // Latest request owner, used to discard old-session responses
...
export const fetchGamesBySection = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      const ownerKey = getGameCacheOwner(user);
      ...
      if (!force && !background) {
        ...
        const cacheOwner = state.games.gamesBySectionOwner[uiSection];
        const cacheTTL = state.games.gamesBySectionCacheTTL;

        if (cachedGames && cacheTimestamp && ownerKey && cacheOwner === ownerKey) {
          const cacheAge = Date.now() - cacheTimestamp;
      ...
      // A response started under a previous account must never update the next
      // account's Redux state.
      if (localStorage.getItem("authToken") !== authToken) {
        throw new Error("Authentication session changed while loading games");
      }
...
const gameSlice = createSlice({
  ...
  reducers: {
    ...
    clearGames: (state) => {
      ...
      state.gamesBySectionOwner = {};
      state.gamesBySectionRequestOwner = {};
      state.availableUiSections = [];
      state.mostPlayedScreenGames = [];
      state.mostPlayedScreenStatus = "idle";
      state.mostPlayedScreenError = null;
      state.mostPlayedScreenCacheTimestamp = null;
    },
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchGamesBySection.pending, (state, action) => {
        ...
        const isBackground = action.meta.arg.background || false;
        const ownerKey = getGameCacheOwner(action.meta.arg.user);
        if (ownerKey) {
          state.gamesBySectionRequestOwner[uiSection] = ownerKey;
          if (state.gamesBySectionOwner[uiSection] !== ownerKey) {
            state.gamesBySection[uiSection] = [];
            delete state.gamesBySectionTimestamp[uiSection];
            delete state.gamesBySectionOwner[uiSection];
          }
        }
      ...
      .addCase(fetchGamesBySection.fulfilled, (state, action) => {
        ...
        const isBackground = action.meta.arg.background || false;
        const fromCache = action.payload.fromCache || false;
        const ownerKey = action.payload.ownerKey || null;
        if (!ownerKey) return;
        const requestOwner = state.gamesBySectionRequestOwner[uiSection];
        if (ownerKey && requestOwner && ownerKey !== requestOwner) return;
        ...
        if (!fromCache) {
          ...
          state.gamesBySectionOwner[uiSection] = ownerKey;
          // Drop blank/invalid section names from the API. An empty-string
          // entry ("" in uiSections, from a section saved without a name)
          // caused an infinite fetch loop: components iterating this list saw
          // status "idle" for "" forever because reducers key that fetch under
          // "Unknown", never under "" (BUG_017 freeze, 60+ req/s).
          const validSections = Array.isArray(action.payload.uiSections)
            ? action.payload.uiSections.filter(
      ...
      .addCase(fetchBitlabsDownloadedGames.fulfilled, (state, action) => {
        ...
        const mappedGames = bitlabsGames.map((game) => {
          ...
          // Fallback: if per-event days_left is null, try game-level time fields
          // NOTE: hours_left = 0 means expired, null/undefined = no time limit data
          if (goals.some((g) => g.days_left == null)) {
            let gameDaysLeft = null;
            if (game.hours_left != null) {
              gameDaysLeft = Math.ceil(game.hours_left / 24);
            } else if (game.session_hours > 0) {
              gameDaysLeft = Math.ceil(game.session_hours / 24);
            } else if (game.sessionHours > 0) {
              gameDaysLeft = Math.ceil(game.sessionHours / 24);
            } else if (game.latest_date != null) {
              const expiresMs = new Date(game.latest_date).getTime();
              if (!isNaN(expiresMs)) {
                gameDaysLeft = Math.ceil(
                  (expiresMs - Date.now()) / (1000 * 60 * 60 * 24),
                );
              }
            }
            if (gameDaysLeft != null) {
              goals.forEach((g) => {
                if (g.days_left == null) g.days_left = gameDaysLeft;
              });
            }
          }
      ...
      .addCase(fetchBitlabsAIDownloadedGames.fulfilled, (state, action) => {
        ...
        const mappedAIGames = bitlabsAIGames.map((game) => {
          ...
          return {
            ...
            hoursLeft: game.hours_left,
            days_left: game.hours_left != null ? Math.ceil(game.hours_left / 24) : null,
            source: "bitlabs_ai",
