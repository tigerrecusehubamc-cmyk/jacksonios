...
const initialState = {
  ...
  streakHistory: [],
  // Fields from /api/streak/status response
  streakTree: [], // 30-day grid — each item: { day, isCompleted, isMilestone, rewards, isCurrent }
  rewards: [], // upcoming unclaimed milestone rewards
  progress: null, // { current, target, percentage }
  nextMilestone: null, // { day: N }
  lastMilestone: null, // { day: N }
  isActive: false,
  daysRemaining: 0,
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchStreakStatus.fulfilled, (state, action) => {
        ...
        // Store new fields from /api/streak/status
        state.streakTree = action.payload.streakTree || [];
        state.rewards = action.payload.rewards || [];
        state.progress = action.payload.progress || null;
        state.nextMilestone = action.payload.nextMilestone || null;
        state.lastMilestone = action.payload.lastMilestone || null;
