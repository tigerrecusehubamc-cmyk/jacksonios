...
// --- Replay Protection: Nonce + Secure Enclave Signing ---
// GET /api/biometric/nonce - Request a nonce for replay protection
export const requestBiometricNonce = async (identifier) => {
  try {
    apiLog("🔍 [API] requestBiometricNonce:", identifier);
    const result = await apiRequest(
      `/api/biometric/nonce?identifier=${encodeURIComponent(identifier)}`,
      "GET",
    );
    return result;
  } catch (error) {
    apiError("❌ [API] requestBiometricNonce error:", error);
    return {
...
// POST /api/biometric/biometric-login - Now supports replay protection with nonce + signature
// Expected payload: { identifier, deviceId, biometricType, nonce?, signature?, publicKey? }
...
/**
 * Verify and confirm Apple In-App Purchase
 * @param {object} data - { transactionReceipt, transactionId, productId, subscriptionId, metadata }
 * @param {string} token - The user's authentication token
 */
export const verifyApplePurchase = (data, token) => {
  const requestBody = {
    appStoreReceipt: data.transactionReceipt,
    // StoreKit 2 signed transaction - accepted by the backend when the legacy
    // receipt file does not exist (common in TestFlight/sandbox)
  ...
  return apiRequest("/api/vip/membership/complete-purchase", "POST", requestBody, token);
};

export const initiateApplePurchase = (data, token) =>
  apiRequest("/api/vip/membership/initiate-purchase", "POST", data, token);

/**
 * Get active Apple IAP subscription
 * @param {string} token - The user's authentication token
 */
export const getActiveAppleSubscription = (token) =>
  apiRequest("/api/apple-iap/subscription/active", "GET", null, token);

/**
 * Get Apple IAP purchase history
 * @param {number} limit - Number of records to return
 * @param {string} token - The user's authentication token
 */
export const getApplePurchaseHistory = (limit = 20, token) =>
  apiRequest(`/api/apple-iap/history?limit=${limit}`, "GET", null, token);
...
/**
 * Report play time toward today's game challenge.
 *
 * Play-time challenges are validated server-side against minutes reported here
 * and nowhere else - the server deliberately does not trust cumulative game
 * duration or a first/last-played span, both of which measure elapsed
 * wall-clock rather than play. Without this call a play-time challenge can
 * never be completed.
 *
 * @param {number} playTimeMinutes - Minutes played in this session
 * @param {string} token - User authentication token
 */
export const reportChallengePlayTime = (playTimeMinutes, token) => {
  return apiRequest(
    "/api/daily-challenge/update-progress",
    "PUT",
    { playTimeMinutes },
    token,
  );
