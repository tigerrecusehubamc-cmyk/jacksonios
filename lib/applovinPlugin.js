 ...
 * - iOS Rewarded: 8c95835c30da8a2b (created 2026-08-21 under com.jackson.rewards.app; previous unit b3bf29fc650db20d belonged to a different app and returned MAX -5603)
...
class AppLovinMAXPlugin {
  constructor() {
    ...
    this.nativeListenersSetup = false;
  }
  ...
  async initialize(config = {}) {
    ...
    this.initializationPromise = this.initializeInternal(config);
    try {
  ...
  async initializeInternal(config = {}) {
    ...
    try {
      ...
      if (isNative) {
        ...
        if (pluginInfo) {
          ...
          throw new Error('AppLovin native plugin is not available');
        }
      } else {
  ...
  setupCordovaListeners() {
    ...
    window.addEventListener('OnRewardedAdLoadFailedEvent', (event) => {
      ...
      this.adFailListeners.forEach(listener => listener({
        type: 'load',
        error: errorInfo.error || errorInfo.message || 'Failed to load ad',
        errorCode: errorInfo.errorCode || errorInfo.code || 'LOAD_ERROR',
      }));
    });
    ...
    window.addEventListener('OnRewardedAdFailedToDisplayEvent', (event) => {
      ...
      this.adFailListeners.forEach(listener => listener({
        type: 'display',
        error: errorInfo.error || errorInfo.message || 'Failed to display ad',
        errorCode: errorInfo.errorCode || errorInfo.code || 'DISPLAY_ERROR',
      }));
    });
   ...
   */
  async loadAd() {
    if (this.isAdLoaded) return this.currentAdInfo;
    if (this.loadPromise) return this.loadPromise;

    this.loadPromise = this.loadAdInternal();
    try {
          ...
          const onFailed = (error) => {
            ...
            const errorMsg = error?.error || error?.message || 'Failed to load ad';
              ...
              if (isNetworkError && retryCount < maxRetries) {
                ...
                this.removeLoadListener(onLoaded);
                this.removeFailListener(onFailed);
                const loadError = new Error(finalErrorMsg);
                loadError.errorCode = error?.errorCode || error?.code;
                loadError.adUnitId = error?.adUnitId || this.adUnitId;
                reject(loadError);
              }
          };
  ...
  addLoadListener(listener) {
    if (!this.adLoadListeners.includes(listener)) this.adLoadListeners.push(listener);
    return () => this.removeLoadListener(listener);
  }
  ...
  addDisplayListener(listener) {
    if (!this.adDisplayListeners.includes(listener)) this.adDisplayListeners.push(listener);
    return () => this.removeDisplayListener(listener);
  }
  ...
  addCompleteListener(listener) {
    if (!this.adCompleteListeners.includes(listener)) this.adCompleteListeners.push(listener);
    return () => this.removeCompleteListener(listener);
  }
  ...
  addFailListener(listener) {
    if (!this.adFailListeners.includes(listener)) this.adFailListeners.push(listener);
    return () => this.removeFailListener(listener);
  }
