...
import { isAlreadyFired, markFiredOnServer } from "./adjustCounters";
...
const fireEvent = (
  ...
  // Always include user_id in SDK callback params for webhook matching
  const userId = getUserIdFromToken();
  if (userId) {
    if (!sdkParams.callbackParams) sdkParams.callbackParams = [];
    if (!sdkParams.callbackParams.some(p => p.key === "user_id")) {
      sdkParams.callbackParams.push({ key: "user_id", value: userId });
    }
  }
...
export const onGameDownload = (totalDownloads) => {
  ...
  if (!key || isAlreadyFired(key)) return;
  markFiredOnServer(key);
  fireEvent(
    SDK_TOKENS[key],
    S2S[key],
    {},
...
export const onInGamePurchase = (purchaseCount, revenue, productId) => {
  ...
  if (countKey && !isAlreadyFired(countKey)) {
    markFiredOnServer(countKey);
    fireEvent(
      SDK_TOKENS[countKey],
      S2S[countKey],
      {},
  ...
  if (revenueKey && !isAlreadyFired(revenueKey)) {
    markFiredOnServer(revenueKey);
    fireEvent(
