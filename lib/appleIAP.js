...
export async function purchaseSubscription(productId) {
  try {
    ...
    // StoreKit 2 purchases (TestFlight/sandbox especially) often have no legacy
    // receipt file, so `receipt` is missing even for a successful charge. The
    // signed transaction (JWS) is always present on iOS 15+ and the backend
    // verifies it against Apple's CA, so either proof is acceptable.
    const jwsRepresentation = result.jwsRepresentation
    ...
    if (!transactionReceipt && !jwsRepresentation) {
