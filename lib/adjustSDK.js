 ...
 * On Android (Capacitor), the native SDK is already initialised in MainApplication.java.
...
export const initAdjustSDK = async () => {
  ...
  // On Android native — native SDK already running, skip Web SDK init
  if (isCapacitorNative()) {
    console.log("[Adjust SDK] Running on Android native — Web SDK skipped, using native SDK");
...
export const trackSDKEvent = async (token, params = {}) => {
  ...
  // ── Android native path (Capacitor bridge → native Adjust SDK) ──────────
  if (isCapacitorNative()) {
    try {
