 ...
 * On native (Capacitor), the native SDK may already be initialised natively.
 * This Web SDK init only runs on web/browser.
 * Safe to call multiple times — subsequent calls are no-ops.
 */
export const initAdjustSDK = async () => {
  if (typeof window === "undefined") return;

  // On native — skip Web SDK init (native SDK handles it)
  if (isCapacitorNative()) {
    console.log("[Adjust SDK] Running on native — Web SDK skipped, using native SDK");
 ...
 * On native (Capacitor) → uses native Adjust SDK via Capacitor bridge (proper GAID attribution).
...
export const trackSDKEvent = async (token, params = {}) => {
  ...
  // ── Native path (Capacitor bridge → native Adjust SDK) ──────────────────
  if (isCapacitorNative()) {
    try {
  ...
  // ── Web SDK fallback (browser / non-native) ──────────────────────────────
  try {
    const { default: Adjust } = await import("@adjustcom/adjust-web-sdk");
