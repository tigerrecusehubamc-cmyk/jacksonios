...
export const getAdvertisingId = async () => {
  ...
  try {
    const { AdvertisingId } = await import("@capacitor-community/advertising-id");
    ...
    // @capacitor-community/advertising-id returns { id, status }
    // where status = 'Authorized' | 'Denied' | 'Not Determined' | 'Restricted'
    const type = result.status === "Authorized"
      ? (window.Capacitor.getPlatform() === "ios" ? "idfa" : "gps_adid")
      : null;
